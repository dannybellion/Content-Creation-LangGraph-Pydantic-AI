...
async def analyze_readability(ctx: RunContext[str], content: str) -> str:
    """
    ...
    try:
        ...
        recommendations = _get_readability_recommendations(
            flesch_score, avg_sentence_length
        )
        ...
        return str(
            {
                ...
                "total_sentences": sentences,
                "recommendations": [
                    "Could not perform detailed readability analysis"
                ],
            }
        )
...
def _get_readability_recommendations(
    flesch_score: float, avg_sentence_length: float
) -> List[str]:
    ...
    if avg_sentence_length < 8:
        recommendations.append(
            "Sentences may be too short - consider combining some for better flow"
        )
...
async def check_keyword_density(
    ctx: RunContext[str], content: str, keywords: List[str]
) -> str:
    """
    ...
    for keyword in keywords:
        ...
        keyword_analysis[keyword] = {
            ...
            "word_counts": dict(zip(keyword_words, word_counts))
    ...
    return str(
        {
            ...
            "overall_assessment": _assess_overall_keyword_optimization(
                total_keyword_density, len(keywords)
            ),
        }
    )


def _assess_overall_keyword_optimization(
    total_density: float, keyword_count: int
) -> str:
...
async def generate_meta_description(
    content: str, keywords: List[str] = None, max_length: int = 160
) -> str:
    """
    ...
    if len(first_paragraph) > max_length:
        sentences = [
            s.strip() for s in first_paragraph.split(".") if s.strip()
        ]
        meta = sentences[0]
        ...
        for sentence in sentences[1:]:
            if (
                len(meta + ". " + sentence) < max_length - 3
            ):  # Leave room for "..."
                meta += ". " + sentence
            else:
                break
    ...
    if keywords and keywords[0].lower() not in meta.lower():
        ...
        if len(keyword_prefix + meta) <= max_length:
            ...
            meta = (
                meta[: max_length - len(keywords[0]) - 10]
                + f" about {keywords[0]}"
            )
...
async def estimate_reading_time(
    content: str, words_per_minute: int = 225
) -> Dict[str, Any]:
    """
...
async def analyze_content_structure(content: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if line.startswith("#") or (
            len(line) < 100 and len(line.split()) < 12 and line.endswith(":")
        ):
    ...
    return {
        ...
        "has_good_structure": len(headings) >= 3
        and avg_paragraph_length < 100,
...
def _get_structure_recommendations(
    ...
    if heading_count < 3:
        recommendations.append(
            "Add more headings to break up content and improve scannability"
        )
    ...
    if list_count == 0 and paragraph_count > 5:
        recommendations.append(
            "Consider using bullet points or numbered lists for key information"
        )

    if heading_count > paragraph_count:
        recommendations.append(
            "Consider combining some sections or adding more content under headings"
        )

    if not recommendations:
        recommendations.append(
            "Content structure looks good - well-organized and scannable"
        )
